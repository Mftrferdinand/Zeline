# sk30 — Client Revenue Engine (Zeline Zenith)
...
| **B. Mass akun / airdrop / on-chain** | BulkRunner + Checkpoint (resume) + dedupe | sk10 + zeline (`wallet_manager`, `airdrop_runner`) — **WAJIB lewat `governor.py`** |
...
- Jangan harvest data pribadi/PII di luar izin klien. Simpan kredensial klien di vault (`zeline/references/security.md`), bukan hardcode.
