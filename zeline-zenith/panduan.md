# Panduan Operator — Zeline + Zeline Zenith v7.0 ZENITH SUPREME

Cara ngomong sama Zeline setelah Zeline Zenith v7.0 + zeline-crypto-agent ke-combo.
...
```
Idenya: input guard periksa prompt *kamu* (lawan: parafrase intent), output guard periksa jawaban *model* (lawan: jangan emit secret literal → encode/acrostic/eja). Detail teknik & defense ladder L1–L8 ada di `skills/zeline-zenith-sk48.md`. ⚠️ API contract Gandalf gak resmi — verify dulu di DevTools sebelum jalan.
...
|---|---|---|
| "mint NFT [URL]" | sk13 | `skills/zeline-zenith-sk13.md` |
...
| "audit kontrak [addr]" | sk11 | `skills/zeline-zenith-sk11.md` |
| "kenapa bot gua duplikat" | sk54 + sk4 | debug + telegram |
| "deploy ke VPS" | sk2 | `skills/zeline-zenith-sk2.md` |
| "buat landing page web3" | sk9 + sk10 | frontend + Web3 connect |
| "bikin animasi penjelasan / diagram" | sk18 | `skills/zeline-zenith-sk18.md` (Manim/Excalidraw) |
...
| "pecahin task ini, gua overwhelmed" | sk23 | `skills/zeline-zenith-sk23.md` |
| "bikin MCP server" | sk24 | sk24 + `mcp_builder.py` |
| "cek prompt gua ada masalah gak" | sk24 | sk24 (`audit_prompt`) |
| "review compliance / CI-CD / port kode" | sk25 | `skills/zeline-zenith-sk25.md` |
...
| "debug bug aneh yang kadang muncul" | sk57 | `skills/zeline-zenith-sk57.md` (4-fase + variance) |
| "tujuan gua masih kabur, bantu rapihin" | sk58 | `skills/zeline-zenith-sk58.md` |
...
| "tulis pakai framework AIDA/PAS" | sk28 | `skills/zeline-zenith-sk28.md` |
...
| "skor eligibility wallet buat [proyek]" | sk31 | `skills/zeline-zenith-sk31.md` + `eligibility.py` |
...
| "triage soal CTF / decode flag" | sk32 | `skills/zeline-zenith-sk32.md` + `ctf.py` |
| "caesar / xor / hash tipe apa" | sk32 | sk32 + `ctf.py` |
| "solve CTF web (SQLi/SSTI/JWT)" | sk32 → sk43 | `skills/zeline-zenith-sk43.md` + `tools/ctf/` |
| "pwn binary / ret2libc / heap" | sk32 → sk44 | `skills/zeline-zenith-sk44.md` + pwntools template |
| "reverse binary / angr / keygen" | sk32 → sk45 | `skills/zeline-zenith-sk45.md` + angr template |
| "RSA attack / crypto challenge" | sk32 → sk46 | `skills/zeline-zenith-sk46.md` + `tools/ctf/rsa_attacks.py` |
| "forensics / stego / pcap / memdump" | sk32 → sk47 | `skills/zeline-zenith-sk47.md` + sandbox tools |
| "prompt injection / jailbreak / Gandalf" | sk32 → sk48 | `skills/zeline-zenith-sk48.md` + `tools/ctf/gandalf_solver.py` |
| "full-auto solve CTFd (swarm)" | sk32 | `tools/ctf/run.py` (coordinator + sandbox) |
| "scan proyek pre-TGE / alpha airdrop" | sk33 | `skills/zeline-zenith-sk33.md` + `alpha_radar.py` |
...
1. **VPS akses** — ✅ udah punya `root@<vps-host>`
...
# harus muncul: [BOOT] v7.0 ZENITH SUPREME loaded
...
Kalau 5 hal di atas ✅, Zeline Zenith v7.0 siap eksekusi.
