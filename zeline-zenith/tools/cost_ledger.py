...
Desain:
- SQLite tunggal (default ~/.zeline/ledger.db) → tahan restart, zero-dep.
...
DEFAULT_DB = Path(os.environ.get("ZELINE_ZENITH_LEDGER_DB", "~/.zeline/ledger.db")).expanduser()

# Estimasi USD per 1k token (input+output digabung). Operator override via env JSON:
#   ZELINE_ZENITH_PRICE_PER_1K='{"claude":0.009,"gpt-4o":0.005}'
...
def _load_price_map() -> dict:
    raw = os.environ.get("ZELINE_ZENITH_PRICE_PER_1K")
    if raw:
        try:
