...
m32 buat **CTF legal & bug-bounty in-scope**: platform latihan (HackTheBox/TryHackMe/CTFd), kompetisi resmi, atau target yang operator punya **izin tertulis**. Tujuannya dapet reward/skill secara whitehat.
...
- **web**: cek header/cookie/JWT, params (IDOR/LFI/SQLi), source map, robots.txt — pakai m11 + httpx, izin in-scope only.
...
m32 = **router CTF**. Buat soal nyata (bukan cuma decode), m32 triage → cek scope → dispatch ke sub-skill kategori → validasi flag → writeup. Sub-skill kategori:

| Sinyal soal | Sub-skill |
|---|---|
| URL / web app / HTTP service | **m43** (ctf-web) |
| ELF/PE + remote `nc host port` | **m44** (ctf-pwn) |
| ELF/PE, "what does it do", no overflow | **m45** (ctf-rev) |
| ciphertext / key / `.pem` / math / encoding | **m46** (ctf-crypto) |
| `.pcap` / memory dump / image / zip / "hidden" | **m47** (ctf-forensics) |
| LLM/AI target, jailbreak, prompt injection, Gandalf/Lakera | **m48** (ctf-prompt-injection) |
| gak jelas | m47 file-id dulu → re-triage |
...
| Web / pwn / rev / crypto / forensics dalam | dispatch | m43 / m44 / m45 / m46 / m47 |
| LLM red-team / prompt injection (Gandalf) | dispatch | m48 (`tools/ctf/gandalf_solver.py`) |
...
| Audit kerentanan target | recon checklist | m11 (audit/exploit review) |
| HTTP recon in-scope | — | m6 (httpx) + m11 |
| Otomasi banyak soal/round | — | m12/m30 (batch, rate-limit) |
| Writeup/report reward | — | m8 (PDF) / m26 (spec) |
...
```
🔧 Upgrade: combo m8 buat auto-generate writeup PDF tiap flag ketemu.
