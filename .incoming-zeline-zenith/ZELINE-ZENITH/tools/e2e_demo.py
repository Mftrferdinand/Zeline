#!/usr/bin/env python3
"""
tools/e2e_demo.py — ZELINE ZENITH End-to-End Pipeline Demo
═══════════════════════════════════════════════════════════════════════

Proves the full ZELINE ZENITH pipeline works end-to-end:
...
ROOT_DIR = PKG_DIR.parent                            # ZELINE ZENITH/
...
def run_stage_auth(config_path: Path) -> Dict[str, Any]:
    ...
    print(f"    {_c('  Format:', Color.DIM)} ZELINE ZENITH-V7-AUTH:{{timestamp}}:{{nonce}}")
...
def print_final_summary(auth_data, bridge_data, revenue_data, treasury_data):
    ...
    print(_c("  🏆  ZELINE ZENITH — END-TO-END PIPELINE DEMO  🏆", Color.BOLD + Color.CYAN))
    ...
    if all_stages_ok:
        print(_c("  ✅  ALL 4 STAGES PASSED — ZELINE ZENITH PIPELINE VERIFIED", Color.GREEN + Color.BOLD))
    else:
...
def main():
    ...
    print(_c("  ZELINE ZENITH — END-TO-END PIPELINE DEMO", Color.CYAN + Color.BOLD))
