# DEPLOY.md — Pasang ZELINE ZENITH di Zeline (dari nol)
...
> Ini **menggantikan** instruksi quick-start lama di README. Jalur workspace V7: `~/.zeline/workspace/zeline-zenith/`.
...
cp -r zeline ~/.zeline/workspace/zeline-zenith
...
```json
{
  "agents": {
    "defaults": {
      "workspace": "~/.zeline/workspace/zeline-zenith"
    }
  }
}
```
...
```bash
cd ~/.zeline/workspace/zeline-zenith
...
```
"siapa lo?"            → harusnya jawab sebagai ZELINE ZENITH SUPREME
...
0 7 * * *  cd ~/.zeline/workspace/zeline-zenith && python -c "..."   # wire push_briefing ke notifier lo
...
```
☐ zeline setup → workspace = ~/.zeline/workspace/zeline-zenith
