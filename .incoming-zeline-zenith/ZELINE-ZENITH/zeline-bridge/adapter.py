#!/usr/bin/env python3
"""
zeline-bridge/adapter.py — ZELINE ZENITH ↔ Zeline Runtime Bridge
Maps ZELINE ZENITH tools (sk*) to Zeline function calls.
Enables Zeline-native execution of ZELINE ZENITH crypto ops.
...
Architecture:
  ZELINE ZENITH skills (sk10/sk13/H1-H10) → Zeline dispatch → zeline/scripts/*.py
  This adapter provides a unified CLI that routes to the right script.

v2.0 — Adds ZelineNativeBridge (direct Python import, no subprocess) +
       ZelineMCPTool dataclass + OpenAI function-calling schema generator.
       ZelineBridgeLegacy (subprocess) kept as fallback.
"""
...
# ─── Zeline paths ───
ZELINE_ROOT = Path(os.environ.get("ZELINE_ROOT", Path.home() / ".zeline"))
ZELINE_SCRIPTS = ZELINE_ROOT / "skills" / "zeline" / "scripts"
ZELINE_ZENITH_ROOT = Path(__file__).parent.parent  # zeline/ root

# ─── Tool mapping: ZELINE ZENITH → Zeline script ───
TOOL_MAP = {
    "swap": {
...
@dataclass
class BridgeResult:
    """Result from Zeline bridge operation."""
    success: bool
    tool: str
    action: str
...
@dataclass
class ZelineMCPTool:
    """MCP-compatible tool definition for Zeline runtime.
    ...
    def execute(self, bridge: "ZelineNativeBridge", arguments: dict) -> BridgeResult:
        """Execute this tool via the provided bridge instance.

        Args:
            bridge: ZelineNativeBridge instance
...
# ══════════════════════════════════════════════════════════════════════
# ZelineBridgeLegacy — subprocess-based (renamed from ZelineBridge)
# ══════════════════════════════════════════════════════════════════════

class ZelineBridgeLegacy:
    """Adapter between ZELINE ZENITH and Zeline runtime (subprocess CLI)."""

    def __init__(self, zeline_root: Optional[Path] = None):
        self.zeline_root = zeline_root or ZELINE_ROOT
        self.scripts_dir = self.zeline_root / "skills" / "zeline" / "scripts"
    ...
    def execute(self, tool: str, args: dict) -> BridgeResult:
        """Execute a tool through Zeline bridge (subprocess)."""
        info = TOOL_MAP.get(tool)
        if not info:
        ...
        if not script_path.exists():
            return BridgeResult(
                ...
                error=f"Script not found: {script_path}. "
                      f"Make sure Zeline is installed at {self.zeline_root}"
            )
    ...
    def _verify_integrity(self) -> BridgeResult:
        ...
        integrity_script = ZELINE_ZENITH_ROOT / "tools" / "skill_integrity.py"
    ...
    def status(self) -> dict:
        ...
        return {
            ...
            "zeline_root": str(self.zeline_root),
            ...
            "zenith_root": str(ZELINE_ZENITH_ROOT),
        }


# ══════════════════════════════════════════════════════════════════════
# ZelineNativeBridge — Direct Python import, no subprocess
# ══════════════════════════════════════════════════════════════════════
...
class ZelineNativeBridge:
    """Native Python bridge — imports Zeline scripts directly, no subprocess.

    Advantages over ZelineBridgeLegacy:
    ...
    def __init__(self, zeline_root: Optional[Path] = None):
        ...
        self._fallback_legacy: Optional[ZelineBridgeLegacy] = None

    @property
    def legacy(self) -> ZelineBridgeLegacy:
        """Lazy fallback to subprocess-based legacy bridge."""
        if self._fallback_legacy is None:
            self._fallback_legacy = ZelineBridgeLegacy(self.zeline_root)
        return self._fallback_legacy

    def _resolve_scripts_dir(self) -> Optional[Path]:
        """Find zeline scripts directory from multiple possible locations.

        Precedence:
        1. Configured self.scripts_dir
        2. Path relative to THIS file (zeline-bridge/../skills/zeline/scripts/)
        3. ZELINE_ZENITH_ROOT/skills/zeline/scripts/
        """
        if self.scripts_dir.exists():
            return self.scripts_dir

        # Auto-detect relative to this adapter
        bridge_dir = Path(__file__).parent  # zeline-bridge/
        adjacent = bridge_dir.parent / "skills" / "zeline" / "scripts"
        if adjacent.exists():
            return adjacent

        # ZELINE_ZENITH_ROOT fallback
        sa_root = Path(__file__).parent.parent
        sa_scripts = sa_root / "skills" / "zeline" / "scripts"
    ...
    def _ensure_path(self) -> Path:
        """Ensure zeline scripts are on sys.path for direct imports."""
        scripts_dir = self._resolve_scripts_dir()
        if scripts_dir is None:
            raise RuntimeError(
                f"Zeline scripts not found. Tried: {self.scripts_dir}, "
                f"{Path(__file__).parent.parent / 'skills' / 'zeline' / 'scripts'}"
            )
    ...
    def _import_module(self, script_name: str):
        """Import a zeline script module directly (cached)."""
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to import zeline script '{script_name}': {e}. "
                f"Falling back to ZelineBridgeLegacy."
            ) from e
    ...
    def call_tool(self, tool_name: str, arguments: dict) -> BridgeResult:
        ...
        Falls back to ZelineBridgeLegacy if native execution fails.
    ...
    def get_tools(self) -> list[ZelineMCPTool]:
        """Return MCP-compatible tool definitions for Zeline runtime."""
        tools = []
        for tool_name, info in TOOL_MAP.items():
            params = TOOL_PARAMETER_SCHEMAS.get(tool_name, {
                "type": "object",
...
# Existing code using `ZelineBridge` gets the new native bridge.
# Use `ZelineBridgeLegacy` explicitly to force subprocess mode.
ZelineBridge = ZelineNativeBridge
...
if __name__ == "__main__":
    ...
    if use_legacy:
        ...
        bridge = ZelineBridgeLegacy()
    else:
        bridge = ZelineNativeBridge()
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("--help", "-h"):
        ...
        print("\nFlags:")
        print("  --legacy      Use subprocess-based ZelineBridgeLegacy (v1 fallback)")
    ...
    if cmd == "status":
        ...
        if isinstance(bridge, ZelineNativeBridge):
        ...
        # Export full ZelineMCPTool definitions
        if isinstance(bridge, ZelineNativeBridge):
