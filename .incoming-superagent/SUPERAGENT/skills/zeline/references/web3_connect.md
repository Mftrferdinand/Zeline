...
Reference untuk integrasi Zeline ke dApp: Sign-In with Ethereum (SIWE), Sign-In with Solana (SIWS), WalletConnect v2, EIP-712 typed data signing, dan EIP-1271 (smart contract wallet) verification.
...
### Verifikasi SIWE (sisi server, kalau Zeline jadi authenticator)
...
WalletConnect = bridge antara dApp (browser) dan wallet (mobile/agent). Zeline bertindak sebagai **wallet** (yang menerima request).
...
WalletConnect adalah vector phishing #1. Sebelum sign request dari session WC, Zeline wajib:
...
## Confirmation Template (untuk Zeline)
