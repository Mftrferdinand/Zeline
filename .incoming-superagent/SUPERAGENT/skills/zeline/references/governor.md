...
```bash
export ZELINE_MAX_TX_USD=500          # cap nilai 1 tx
export ZELINE_DAILY_CAP_USD=2000      # cap 24 jam per wallet
export ZELINE_SESSION_CAP_USD=1000    # cap per sesi (semua wallet)
export ZELINE_MAX_SLIPPAGE_PCT=5      # tolak swap di atas ini
export ZELINE_MAX_GAS_MULTIPLE=4      # gas spike → halt
export ZELINE_MAX_TX_PER_MIN=12       # runaway protection
export ZELINE_REQUIRE_SIM=1           # 0 = matikan (TIDAK disarankan)
export ZELINE_GOVERNOR_DB=~/.zeline/governor.db
export ZELINE_KILLSWITCH=~/.zeline/HALT   # `touch` file ini = halt darurat manual
```
...
```bash
echo "manual halt: liat tx aneh" > ~/.zeline/HALT
```
