"""
zeline/governor.py — Spend Governor / Circuit Breaker  (v4.0)
...
- State persisten di SQLite (default ~/.zeline/governor.db) → tahan restart.
...
DEFAULT_DB = Path(os.environ.get("ZELINE_GOVERNOR_DB", "~/.zeline/governor.db")).expanduser()
KILLSWITCH_FILE = Path(os.environ.get("ZELINE_KILLSWITCH", "~/.zeline/HALT")).expanduser()
...
class GovernorLimits:
    ...
    def from_env(cls) -> "GovernorLimits":
        ...
        return cls(
            max_tx_usd=f("ZELINE_MAX_TX_USD"),
            daily_cap_usd=f("ZELINE_DAILY_CAP_USD"),
            session_cap_usd=f("ZELINE_SESSION_CAP_USD"),
            max_slippage_pct=f("ZELINE_MAX_SLIPPAGE_PCT") or 5.0,
            max_gas_multiple=f("ZELINE_MAX_GAS_MULTIPLE") or 4.0,
            max_tx_per_min=int(f("ZELINE_MAX_TX_PER_MIN") or 12),
            require_simulation=os.environ.get("ZELINE_REQUIRE_SIM", "1") != "0",
        )
...
class SpendGovernor:
    def __init__(self, limits: Optional[GovernorLimits] = None,
        ...
        SQLite di network FS). Set env ZELINE_GOVERNOR_DB ke path bersama, atau
        passing `conn=` langsung. Skema tabelnya stdlib-SQL, portable.
        """
        self.limits = limits or GovernorLimits.from_env()
        self.db_path = db_path
        self.session_id = session_id or os.environ.get("ZELINE_SESSION_ID", str(int(time.time())))
        if conn is not None:
            self._db = conn
        else:
