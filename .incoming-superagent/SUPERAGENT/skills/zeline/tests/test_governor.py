...
class TestGovernorLimitsFromEnv(unittest.TestCase):
    def test_defaults_when_unset(self):
        for k in ("ZELINE_MAX_TX_USD", "ZELINE_DAILY_CAP_USD", "ZELINE_SESSION_CAP_USD",
                  "ZELINE_MAX_SLIPPAGE_PCT", "ZELINE_MAX_GAS_MULTIPLE",
                  "ZELINE_MAX_TX_PER_MIN", "ZELINE_REQUIRE_SIM"):
    ...
    def test_reads_env(self):
        os.environ["ZELINE_MAX_TX_USD"] = "250"
        os.environ["ZELINE_REQUIRE_SIM"] = "0"
        try:
            lim = G.GovernorLimits.from_env()
            self.assertEqual(lim.max_tx_usd, 250.0)
            self.assertFalse(lim.require_simulation)
        finally:
            os.environ.pop("ZELINE_MAX_TX_USD", None)
            os.environ.pop("ZELINE_REQUIRE_SIM", None)
