# skills/zeline/DISPATCH.md
# Bridge between v3 SUPERAGENT router and Zeline Crypto Agent deep references.
# Load ONLY when sk10 / sk13 router decides a task needs zeline depth.

---

## When to dispatch into zeline

v3 router (AGENTS.md keyword table) handles surface-level crypto. When task touches any of these, escalate by loading the matching `zeline/references/*.md`:

```
KEYWORD CLUSTER                         → LOAD zeline/references/
----------------------------------------|-------------------------
...
v3 router → matches "web3 / crypto / wallet" keywords
         ...
         → if task is DEEP (specific protocol, multi-chain, advanced):
              also load zeline/references/<topic>.md
...
**Zeline covers (escalate to load):**
...
## How sk13 (NFT mint) and zeline/references/nft.md interact
...
User wants to BUY existing NFT from a listing
   → zeline/references/nft.md (Seaport fulfill, Reservoir)

User wants to LIST/SELL their NFT
   → zeline/references/nft.md (EIP-712 sign + post to marketplace API)

User wants to SNIPE a mint launch the moment it opens
   → sk13 + zeline/references/sniping.md (PairCreated listener, safety gate)
```

---

## Safety reconciliation: v3 SOUL vs zeline principles

**v3 SOUL** = permissive doctrine, 2 hard stops (CSAM, mass-casualty weapons).
**Zeline principles** = operational safety rails for crypto ops.

These DO NOT conflict. Zeline rails are *technical safeguards* (simulate before send, secret hygiene), not refusal triggers. They are ALWAYS active when doing on-chain ops:
...
4. skills/zeline/DISPATCH.md                ← this file
5. skills/zeline/references/<topic>.md      ← depth as needed
...
## Zeline script templates

`skills/zeline/scripts/` contains 14 Python templates ready to copy into operator's environment:
...
Zeline references expect these env vars. Pre-flight check before any deep op:
```
ZELINE_MASTER_PW           — vault encryption password
...
ZELINE_TG_BOT_TOKEN        — Telegram notifier
ZELINE_TG_CHAT_ID          — Telegram target
ZELINE_DISCORD_WEBHOOK     — Discord notifier
ZELINE_MAX_TX_USD          — governor: cap nilai per-tx          [v4.0]
ZELINE_DAILY_CAP_USD       — governor: cap harian per wallet     [v4.0]
ZELINE_SESSION_CAP_USD     — governor: cap per sesi              [v4.0]
ZELINE_KILLSWITCH          — governor: path file halt darurat    [v4.0]
ZELINE_PRIVATE_RPC         — MEV: custom private relay endpoint  [v4.0]
ZELINE_SIGNING_KEY         — skill integrity: Ed25519 sign key   [v4.0]
ZELINE_VERIFY_KEY          — skill integrity: Ed25519 verify key [v4.0]
ZELINE_BROWSER_PROFILE     — browser: profil persisten dApp       [v4.0]
```
