...
_DRY = contextvars.ContextVar("zenith_dry_run", default=None)
# Buffer rencana per-run (di-share via list di contextvar biar thread pool ngumpul ke satu tempat)
_PLAN = contextvars.ContextVar("zenith_dry_plan", default=None)


def _env_default() -> bool:
    return os.environ.get("ZELINE_ZENITH_DRY_RUN", "0") not in ("0", "", "false", "False")
