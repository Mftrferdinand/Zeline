#!/usr/bin/env python3
"""
tools/cli.py — Zeline Zenith Unified CLI
...
Usage:
  zeline-zenith scan
  zeline-zenith auth <challenge> <signature>
  zeline-zenith bridge status
  zeline-zenith treasury [daily|weekly|monthly]
  zeline-zenith deploy-gateway
  zeline-zenith dashboard [--serve PORT]
  zeline-zenith eval <label> [--case NAME EXPECT_TYPE VALUE...]
  zeline-zenith watchdog tick
  zeline-zenith reflection cycle
  zeline-zenith triage [--source SOURCE] [--top-k N]
  zeline-zenith integrity verify
  zeline-zenith model list|add|remove
  zeline-zenith upgrade <zip_path> [--dry-run]
  zeline-zenith version
"""
...
def cmd_scan(args) -> int:
    ...
    print("🔍 Zeline Zenith — Revenue Scan")
    try:
...
def cmd_auth(args) -> int:
    ...
    print("🔐 Zeline Zenith — Team Auth Verify")
    ...
    try:
        ...
        print("   ⚠️  eth-account not installed — install with: pip install zeline-zenith[crypto]")
...
def cmd_bridge(args) -> int:
    ...
    print("🌉 Zeline Zenith — ZelineBridge Status")
...
def cmd_treasury(args) -> int:
    ...
    print(f"💰 Zeline Zenith — Treasury Report ({window})")
...
def cmd_deploy_gateway(args) -> int:
    ...
    print("🚀 Zeline Zenith — Deploy Gateway")
...
def cmd_dashboard(args) -> int:
    ...
    try:
        ...
        if args.serve:
            ...
            port = int(args.serve)
            html_path = Path("/tmp/zeline-zenith-dashboard.html")
            ...
            out_path = args.output or "/tmp/zeline-zenith-dashboard.html"
            from pathlib import Path

            Path(out_path).write_text(html)
            print(f"   ✅ Dashboard written to {out_path}")
            print(f"   → Open in browser or serve with: zeline-zenith dashboard --serve 8080")
        return 0
    except ImportError as e:
...
def cmd_eval(args) -> int:
    ...
    print(f"🧪 Zeline Zenith — Eval: {args.label}")
...
def cmd_watchdog(args) -> int:
    ...
    print("🛡️ Zeline Zenith — Watchdog Tick")
...
def cmd_reflection(args) -> int:
    ...
    print("🔄 Zeline Zenith — Reflection Cycle")
...
def cmd_triage(args) -> int:
    ...
    print("📥 Zeline Zenith — Inbox Triage")
...
def cmd_integrity(args) -> int:
    ...
    print("🔒 Zeline Zenith — Skill Integrity")
    ...
    try:
        ...
        if not lock_path.exists():
            print("   ⚠️  SKILLS.lock not found — run 'zeline-zenith integrity generate' first")
            return 1
...
def cmd_upgrade(args) -> int:
    """Run Zeline Zenith upgrade from zip."""
    print("⬆️  Zeline Zenith — Upgrade")
    ...
    try:
        ...
        sys.argv = [
            "zeline-zenith-upgrade",
            args.zip_path,
        ]
        if args.dry_run:
            sys.argv.append("--dry-run")
        if args.list_backups:
            sys.argv = ["zeline-zenith-upgrade", "--list-backups"]
        if args.rollback:
            sys.argv = ["zeline-zenith-upgrade", "--rollback", args.rollback]
...
def cmd_version(args) -> int:
    """Print Zeline Zenith version and tool summary."""
    print(f"🔥 Zeline Zenith — v{VERSION}")
    print(f"   Codename: zenith")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="zeline-zenith",
        description="Zeline Zenith — Sovereign Autonomous Agent Framework CLI",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=textwrap.dedent("""\
            examples:
              zeline-zenith scan                      # run revenue scan
              zeline-zenith treasury monthly          # monthly P&L report
              zeline-zenith dashboard --serve 8080    # serve live dashboard
              zeline-zenith model list                # list registered LLM models
              zeline-zenith version                   # version + tool summary
        """),
    )
    ...
    p_dash.add_argument("--output", "-o", help="Output HTML path (default: /tmp/zeline-zenith-dashboard.html)")
    ...
    # ── upgrade ──
    p_up = sub.add_parser("upgrade", help="Upgrade Zeline Zenith from zip")
