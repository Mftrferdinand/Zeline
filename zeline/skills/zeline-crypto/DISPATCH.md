...
v3 router (AGENTS.md keyword table) handles surface-level crypto. When task touches any of these, escalate by loading the matching `zeline-crypto/references/*.md`:

```
KEYWORD CLUSTER                         → LOAD zeline-crypto/references/
----------------------------------------|-------------------------
...
`skills/zeline-crypto/scripts/` contains 14 Python templates ready to copy into operator's environment:
...
```
Zeline Master PW           — vault encryption password
...
Zeline Signing Key         — skill integrity: Ed25519 sign key   [v4.0]
