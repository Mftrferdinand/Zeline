"""
zeline/mev.py — MEV Protection / Private Transaction Submission  (v4.0)
...
- custom     → set ZELINE_PRIVATE_RPC ke endpoint pilihan (mis. bloXroute, builder sendiri)
...
def resolve_relay(chain_id: int,
    ...
    custom = os.environ.get("ZELINE_PRIVATE_RPC")
    if prefer == "custom" or (custom and prefer not in PRIVATE_RPC):
        if custom:
            return custom, None
        return None, "ZELINE_PRIVATE_RPC belum di-set → fallback public mempool"
    ...
    if chain_id not in PRIVATE_SUPPORTED_CHAINS and not custom:
        return None, (f"chain {chain_id} belum punya private relay terdaftar → "
                      f"fallback public mempool (rawan sandwich). Set ZELINE_PRIVATE_RPC "
