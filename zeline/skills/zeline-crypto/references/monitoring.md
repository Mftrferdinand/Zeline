...
# Lalu di sisi webhook receiver Zeline:
...
async def watch_for_frontrunners(token_address: str, user_addr: str,
                                   pending_tx_hash: str):
    """Setelah Zeline submit swap, watch mempool untuk MEV bot yang try frontrun.
...
```python
async def copy_trade_smart_money(smart_addr: str, zeline_wallet,
                                    max_buy_per_token_eth: float = 0.1,
                                    min_smart_buy_eth: float = 5.0):
    """Watch smart wallet, mirror buy mereka di Zeline wallet (ukuran kecil)."""
    ...
    while True:
        ...
        for tx in transfers:
            ...
            # Zeline mirror buy (jumlah kecil)
            print(f"COPY: smart bought {tx['tokenSymbol']} ({amount_eth} ETH worth)")
            # await swap_evm(w3, zeline_wallet, NATIVE, token, max_buy_per_token_wei)
...
async def floor_drop_alert(collection_slug: str, target_floor_eth: float,
                             notifier: "Notifier"):
    """Buzz Zeline kalau floor turun di bawah target — buy opportunity."""
    while True:
        async with httpx.AsyncClient(
            headers={"x-api-key": os.environ["RESERVOIR_API_KEY"]}
        ) as c:
...
Gabungkan semua jadi alpha bot Zeline:
...
class MonitorState:
    ...
    def __init__(self, db_path: Path = Path.home() / ".zeline" / "monitor.db"):
...
─────────────
Mau cancel tx ini? Zeline bisa kirim replacement tx dengan gas lebih tinggi.
