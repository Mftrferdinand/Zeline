...
Diagnosis terstruktur + debugging sistematis termasuk auto-debug hypothesis loop. zeline-zenith-z91 = pustaka pola error (cepat, lookup) + metode 4-fase buat bug aneh/intermittent.

---

## PART A: FAST DIAGNOSIS (original zeline-zenith-z91)
...
## PART B: SYSTEMATIC DEBUGGING & AUTO-DEBUG LOOP (from z34)

Metode buat bug yang **gak ada di pustaka** — yang aneh, intermittent, atau dalam. Load saat zeline-zenith-z91 §A mentok atau bug non-trivial.

### 4 fase (urut — jangan loncat ke Fix)

```
1. RCA        → Root Cause Analysis: reproduce → isolasi → cari akar, BUKAN gejala
2. PATTERN    → cocokin ke pola dikenal (zeline-zenith-z91 §A library, error sebelumnya di memory/z29 §B)
3. HYPOTHESIS → rumuskan dugaan yang TESTABLE, ranking by likelihood × cheapness-to-test
4. FIX        → uji hipotesis #1; benar → patch + regression test (z33); salah → hipotesis #2
```
...
- Pustaka zeline-zenith-z91 §A (nonce too low, RPC timeout, duplicate webhook, dll).
- Memory/z29 §B — "error ini pernah ketemu?" (lesson reinforced).
...
Untuk bug **intermittent**: pakai `variance()` (z33) — jalanin N kali, lihat distribusi mode kegagalan. Bug yang 2/10 muncul = sinyal race/timing/rate-limit.

### Fase 4 — FIX

```
- Fix akar, bukan gejala. Kalau cuma bisa gejala → tandai + log alasannya (jangan diam).
- Tambah regression test (z33 Eval) — bug yang sama gak boleh balik diam-diam.
- Reversible & operasional (RPC/retry/cache) → boleh auto-fix (z29 §B SAFE_AUTO_ACTIONS).
- Nyentuh skill/dana/rail → proposal (frozen-paths), operator acc + re-lock.
- Catat ke memory (z29 §B): gejala → akar → fix. Next time jadi entry zeline-zenith-z91 §A.
```

### Output ke operator

```
🔬 zeline-zenith-z91 mass-mint flaky (3/50 gagal "AlreadyMinted")
  ...
  FIX: invalidate eligibility cache sebelum batch → regression test added (z33)
  auto-fix? NO (logic skill z14) → 📝 proposal ditulis
```
...
| Bug type | Use |
|---|---|
| Common, well-known error with stack trace | zeline-zenith-z91 §A (fast diagnosis) |
| Unusual, intermittent, no obvious cause | zeline-zenith-z91 §B (systematic 4-fase) |
| zeline-zenith-z91 §A mentok | escalate to zeline-zenith-z91 §B |
| Fix found → needs hardening | zeline-zenith-z91 §B Fase 4 |
