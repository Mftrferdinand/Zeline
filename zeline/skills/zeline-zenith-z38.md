# Contract-Change & Claim-Address Watcher [Zeline Zenith]

> Contract-Change & Claim-Address Watcher — modul Zeline Zenith (sumber: zeline-zenith-z38).
...
Modus drainer paling sering: kontrak/claim address diganti diam-diam pas hype claim. z38 diff dua snapshot kontrak → alert berbobot (info/warning/critical) → keputusan `safe_to_claim`. **Read-only & defensif.**

Pengambilan snapshot on-chain (impl/admin/code hash/ABI) didelegasi ke H9 contract_read + z10. Tool ini CUMA diff — deterministik, offline.
...
| Butuh | z38 | Delegasi |
|---|---|---|
| Snapshot impl/admin/code hash | konsumsi snapshot | H9 contract_read + z10 |
| Verifikasi pengumuman resmi | — | z22 + kanal resmi proyek |
| Eksekusi claim | gate `safe_to_claim` | airdrop_runner + Spend Governor |
| Broadcast warning komunitas | temuan | z37 warning_post + z4 |
...
🔧 Upgrade: cron snapshot harian kontrak yang difarming + auto-alert ke operator via z14.
