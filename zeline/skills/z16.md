# z16 — Daily Assistant II: Watchdog, Vault, Multimodal, Triage (NEW in v4.0)
...
STT lokal gratis: `pip install faster-whisper` (ringan) atau `openai-whisper`. Vision: wire `vision_fn` ke provider multimodal di z8 (Claude/GPT/Gemini). Pattern Telegram (wire di z5): voice→transcribe→route; photo→describe→route. Hasilnya diproses kayak pesan teks biasa (aksi dana tetap lewat governor).
