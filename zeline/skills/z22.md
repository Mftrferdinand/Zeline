...
z22 buat **CTF legal & bug-bounty in-scope**: platform latihan (HackTheBox/TryHackMe/CTFd), kompetisi resmi, atau target yang operator punya **izin tertulis**. Tujuannya dapet reward/skill secara whitehat.
...
z22 = **router CTF**. Buat soal nyata (bukan cuma decode), z22 triage → cek scope → dispatch ke sub-skill kategori → validasi flag → writeup. Sub-skill kategori:
...
| LLM red-team / prompt injection (Gandalf) | dispatch | z28 (`tools/ctf/gandalf_solver.py`) |
...
| Audit kerentanan target | recon checklist | z12 (audit/exploit review) |
| HTTP recon in-scope | — | z7 (httpx) + z12 |
| Otomasi banyak soal/round | — | z13/m30 (batch, rate-limit) |
| Writeup/report reward | — | z9 (PDF) / m26 (spec) |
