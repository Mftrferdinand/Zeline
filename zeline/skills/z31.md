# z31 — Fault Diagnosis, Resolution & Systematic Debugging (v4.2 consolidated)
# MERGED: z31 (fault diagnosis) + z34 (systematic debug/auto-debug loop) — July 2026
...
Diagnosis terstruktur + debugging sistematis termasuk auto-debug hypothesis loop. z31 = pustaka pola error (cepat, lookup) + metode 4-fase buat bug aneh/intermittent.

---

## PART A: FAST DIAGNOSIS (original z31)
...
Metode buat bug yang **gak ada di pustaka** — yang aneh, intermittent, atau dalam. Load saat z31 §A mentok atau bug non-trivial.

### 4 fase (urut — jangan loncat ke Fix)

```
1. RCA        → Root Cause Analysis: reproduce → isolasi → cari akar, BUKAN gejala
2. PATTERN    → cocokin ke pola dikenal (z31 §A library, error sebelumnya di memory/z29 §B)
...
- Pustaka z31 §A (nonce too low, RPC timeout, duplicate webhook, dll).
...
- Catat ke memory (z29 §B): gejala → akar → fix. Next time jadi entry z31 §A.
```

### Output ke operator

```
🔬 z31 mass-mint flaky (3/50 gagal "AlreadyMinted")
...
| Bug type | Use |
|---|---|
| Common, well-known error with stack trace | z31 §A (fast diagnosis) |
| Unusual, intermittent, no obvious cause | z31 §B (systematic 4-fase) |
| z31 §A mentok | escalate to z31 §B |
| Fix found → needs hardening | z31 §B Fase 4 |
