# Power Pack: Planner/Swarm/Automation [Zeline Zenith]

> Power Pack: Planner/Swarm/Automation — modul Zeline Zenith (sumber: zeline-zenith-z17).
...
forge_proposal("belum bisa baca Chainlink oracle", "z18")   # → proposal, REVIEW dulu
```

Agent numbuhin kemampuannya sendiri — tapi lewat proposal (z55/frozen-paths), operator yang acc + re-lock.
...
# serve dari VPS (z2/z9). KASIH AUTH, jangan public polos.
```
...
Dua arah: lo ngomong, agent jawab suara. Lengkapin voice input z15.
...
- Planner/skill-forge/swarm decomposition pakai LLM via `model_registry` (z7/v4.0) — `add model` dulu kalau cascade kosong.
