...
Use `kerjakan langsung (tanpa subagent)` **batch mode** — pass all three tasks in one `tasks`
...
- **Respect project conventions.** If the repo has AGENTS.md / CLAUDE.md /
  ZELINE.md or a linter config, fold those rules into the reviewer prompts so
