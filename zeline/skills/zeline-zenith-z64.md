...
## PART A: CONTENT CREATION (original zeline-zenith-z64)
...
## PART B: CONTENT STRATEGY & SOCIAL MEDIA (from z19)
...
Sebelum produksi, tentuin fondasi (sekali, simpan di vault z16, reuse):
...
# → list {date, pillar, platform, format, topic_slot}  — isi topik via LLM/ide (z21)
```

Distribusi pillar merata + variasi format, hindari spam 1 jenis. Mingguan/bulanan. Sambung cron (z5/z18) buat auto-publish.
...
Hashtag: campur broad (jangkauan) + niche (relevan) + branded (milik lo). Riset trending → z21. Jangan spam 30 tag generik.
...
Repurposing 1 long-form → multi-format → z21 pipeline.
...
- Humanize sebelum publish (m20) biar gak "bau AI". Visual → m18. Frameworks copy → z20.
