# zeline-zenith-z75 — Daily Assistant: Briefing & Alerts (NEW in v4.0)

> zeline-zenith-z75 — Daily Assistant: Briefing & Alerts (NEW in v4.0) — modul Zeline Zenith (sumber: zeline-zenith-z75).
...
from monitoring import Notifier   # dari skills/zeline-crypto/scripts

notifier = Notifier(telegram=(os.environ.get("Zeline TG Bot Token", ""), os.environ.get("Zeline TG Chat ID", "")))
...
`once_per_day=True` (default) ada guard biar gak dobel kalau heartbeat sering. Jadwal: cron harian (z3/z5) atau scheduler in-process.

```bash
# contoh cron jam 7 pagi WIB
0 7 * * *  cd ~/zeline-zenith && python -c "import asyncio; from tools.briefing import ..."  # sesuaikan
```
...
```bash
# Opsional: Zeline Alerts DB (default: ~/.zeline/alerts.db)
# Opsional: Zeline Briefing State (default: ~/.zeline/briefing-last.txt)
# Notifier pakai Zeline TG Bot Token / Zeline TG Chat ID / Zeline Discord Webhook (udah ada)
```
