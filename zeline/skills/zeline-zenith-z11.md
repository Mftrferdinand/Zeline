# Security Audit & Review [Zeline Zenith]

> Security Audit & Review — modul Zeline Zenith (sumber: zeline-zenith-z11).
...
`tools/rugcheck.py` — agregasi sinyal yang udah diambil z11/H4/z10 jadi satu verdict **SAFE / CAUTION / DANGER** + alasan. Murni logika (gak ada network call) → bisa di-eval offline & dipakai jadi blok edukasi konten.
...
- **Unknowns**: data yang BELUM dicek dilaporin eksplisit (jangan diabaikan) — suruh z10/H4 isi dulu.

Pengambilan sinyal didelegasi: honeypot/tax → H4 (GoPlus/honeypot.is), verifikasi/holder/LP → zeline-zenith-z10 (explorer/RPC). rugcheck cuma nge-judge hasilnya jadi keputusan jelas.
