...
- If PNG output fails because `sharp` is missing, use `--out meme.svg` (or another `.svg` path). Do not `npm install` packages into the skill directory or the Zeline tree to enable PNG.

---

## Catatan adaptasi Zeline
- File pendukung tidak di-inline (terlalu besar/biner): references/templates.json, scripts/meme.mjs.
