#!/usr/bin/env python3
"""Google Workspace API CLI for Zeline Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Zeline-facing JSON contract and falls back to the Python client
...
from _zeline_home import get_zeline_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ZELINE_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Zeline Agent")
